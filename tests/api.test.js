...
describe('Sample Test', () => {
  ...
  it('Update checkbox alert', async () => {
    const alert = {
      name: "alerta numero 1",
      description: "lembrar joao",
      date: `${(moment.utc(moment.tz('America/Sao_Paulo').add(3, 'days').format('YYYY-MM-DD')).toDate())}`,
    ...
    const res = await request(app)
      .put(`/alert/updateCheckbox/${alert_id}`)
      .set('x-access-token', token)
      .send(alert);
    expect(res.statusCode).toBe(200);
    expect(res.body.name).toBe(alert.name);
  ...
  it('Update checkbox alert error', async () => {
    const alert = {
      name: "",
      description: "Jest description",
      date: `${(moment.utc(moment.tz('America/Sao_Paulo').add(5, 'days').format('YYYY-MM-DD')).toDate())}`,
  ...
  it('Update checkbox alert with invalid id', async () => {
    const alert = {
      name: "porte de arma",
      description: "avaliação psicológica",
      date: `${(moment.utc(moment.tz('America/Sao_Paulo').add(5, 'days').format('YYYY-MM-DD')).toDate())}`,
    ...
    const res = await request(app)
      .put(`/alert/updateCheckbox/123abc`)
      .set('x-access-token', token)
      .send(alert)
    expect(res.statusCode).toBe(400);
    expect(res.body.err).toBe('invalid id')
  ...
  it('Update checkbox alert without token', async () => {
    const alert = {
      name: "Jest test",
      description: "Jest description",
      date: `${(moment.utc(moment.tz('America/Sao_Paulo').add(5, 'days').format('YYYY-MM-DD')).toDate())}`,
  ...
  it('Update checkbox alert with invalid token', async () => {
    const tokenFalho = 'abc123';
    const alert = {
      name: "Jest test",
      description: "Jest description",
